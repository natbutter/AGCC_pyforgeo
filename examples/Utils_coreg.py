...
def coregRaster(point,data,region):
    ...
    return(scipy.stats.nanmean(pts))
...
def coregLoop(recs,shapes,fields,Nshp):
    '''
    coregLoop
    ...
    timeSteps=20 #The length of time before mineralisation you care about
    noVariables=18 #The number of variables you save
    
    input_rotation_filename = "Shephard_etal_ESR2013_Global_EarthByte_2013.rot"
    ...
    for i, nshpSub in enumerate(xrange(Nshp)):
            ...
            for time in xrange(0,200,1):
                ...
                if  (time > (age-1)) and (time < (age+timeSteps)):
                    ...
                    rasterfile="AgeGrid_20111110_Seton_etal_ESR/agegrid_final_mask_"+str(time)+".grd"
                    [x,y,z]=gridRead(rasterfile)
        
                    #A vector file to coregister with, these points have already been rotated
                    f = readCSV("ConvergenceOutput/subStats_"+str(time)+".csv")   
                    lonlat=f[:,0:2]
